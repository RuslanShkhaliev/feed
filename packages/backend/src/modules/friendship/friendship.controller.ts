...
export class FriendshipController {
    ...
    async followUser(@Req() req, @Param('id') targetUserId: string): Promise<PublicUser> {
        return this.friendshipService.followUser(req.user?.id, Number(targetUserId));
    }

    @Delete(API_ROUTES.FRIENDSHIP.ID)
    public async unfollowUser(@Req() req, @Param('id') targetUserId: string): Promise<PublicUser> {
