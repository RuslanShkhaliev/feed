...
import { createPublicUser } from '@/modules/users/createPublicUser';
...
export class ProfilesService {
  ...
  public async getRecommended(id: number): Promise<PublicUser[]> {
    return this.prismaService.user
      ...
      .then((users) =>
        users.map((user) => createPublicUser(user, false, false)),
      );
  }
  public async getSubscribers(id: number): Promise<PublicUser[]> {
    return this.prismaService.user
      ...
      .then((users) =>
        users.map((user) => createPublicUser(user, true, false)),
      );
  }
  public async getFollowings(id: number): Promise<PublicUser[]> {
    return this.prismaService.user
      ...
      .then((users) =>
        users.map((user) => createPublicUser(user, false, true)),
      );
  }
